...
app.get("/jokes/:id", (req, res) => {
  ...
  const foundJoke = jokes.filter((joke) => joke.id === id);
  res.json(foundJoke);
});
...
app.put("/jokes/:id", (req, res) => {
  ...
  const replacementIndex = jokes.findIndex((joke) => joke.id === replacementId);
...
app.patch("/jokes/:id", (req, res) => {
  const replacementId = parseInt(req.params.id);
  const existingJoke = jokes.find((joke) => joke.id === replacementId);
  const replacementJoke = {
    id : replacementId,
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (deleteIndex > -1) {
    ...
    const removedJoke = jokes.splice(deleteIndex, 1);
    console.log(jokes.length);
    res.status(200).json(removedJoke);
  } else {
    res
      .sendStatus(404)
      .json({ error: `Joke with id: ${deleteID} not found. No jokes were deleted.` }); 
  }
});
...
app.delete("/all", (req, res) => {
  ...
  if (userProvidedKey === masterKey) {
    jokes = [];
    res.sendStatus(200);
    console.log(jokes);
  } else {
    res.status(405).json({ error: `Wrong API Key!`});
  }
});
